...
- [x] 记录 OS、架构、Rust/Node/容器工具版本和缺失工具。
- [x] 记录正式方案 hash、两个 cheetah commit、许可证和契约产物可用性。
- [x] 建立“设计决策不可覆盖清单”和需求追踪表。
- [x] 验证外部依赖可获取；离线环境则记录镜像/包缓存要求。
...
- [x] 检查任务 ID 唯一、前置 ID 存在、依赖无环。
- [x] 检查 `[x]` 任务存在对应报告和 commit 字段。
- [x] 检查相对链接、代码围栏和检查器内置的占位表达式列表。
- [x] 为成功和故意失败 fixture 编写测试。
